# -*- coding: utf-8 -*-
"""
Created on Sat Sep 17 10:12:11 2022
...
def part2(ypred,gt):
   ...
   print("Efficiency/Accuracy of true_value and Random_Y_Value is "+ str((f/a1[0])*100)+ "%")
    

def cifar10_classifier_random(X,Y):

    shape=np.shape(X)
    print("Shape of X_test is "+ str(shape))
    print("It has "+ str(shape[0])+" rows and "+ str(shape[1])+" columns")
    y_pred = [randint(0,9) for _ in range(shape[0])]
    y_shape=np.shape(y_pred)
    print("Array shape of y_pred is "+ str(y_shape))
    print("It has "+ str(y_shape[0])+" rows ")
    print(y_pred)
    part2(y_pred,Y)
    
    
cifar10_classifier_random(X_test, Y_test)
     ...
     # f=0
      #for i in range(0,b1[0]):
       #   if ypred[i] == gt[i]:
        #      f=f+1
      #print("same matched numbers are "+ str(f))        
      #print("Efficiency is "+ str((f/a1[0])*100))
